    ...
    <nav class="bg-white/80 backdrop-blur-md border-b border-gray-100 sticky top-0 z-50">
        <div class="container mx-auto px-6 h-16 flex justify-between items-center flex-row-reverse">
            <div class="flex items-center gap-2">
                ...
                <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest hidden md:inline">E-Library Terintegrasi</span>
                <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest md:hidden">E-Library</span>
            </div>
    ...
    <header class="relative bg-red-700 pt-20 pb-32 overflow-hidden">
        ...
        <div class="container mx-auto px-6 text-center relative z-10">
            ...
            <p class="text-red-100 max-w-xl mx-auto text-[10px] md:text-sm font-medium leading-relaxed opacity-80 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
                Unduh berkas standar untuk menjamin keseragaman administrasi di seluruh unit LDK Al-Fath.
            </p>
        </div>
    </header>
    ...
    <main class="container mx-auto px-6 py-12 -mt-16 relative z-20">
        ...
        <div class="max-w-4xl mx-auto mb-10 animasi-kotak">
            <div class="bg-white p-2 rounded-[32px] shadow-2xl shadow-gray-200/40 border border-gray-100 flex flex-col md:flex-row gap-2 transition-all focus-within:ring-4 focus-within:ring-red-50">
                <div class="flex-1 relative flex items-center">
                    ...
                    <input type="text" id="templateSearch" placeholder="Cari dokumen..." 
                ...
                <div class="bg-gray-50 px-6 py-4 rounded-[24px] flex items-center gap-3">
                    <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Asset:</span>
                    <span class="text-sm font-black text-red-700">{{ $templates->count() }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-4xl mx-auto mb-16 animasi-kotak delay-100">
            <div class="bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group">
                <div class="absolute top-0 right-0 w-32 h-32 bg-amber-50 rounded-full -mr-16 -mt-16 opacity-50 group-hover:scale-110 transition duration-700"></div>
                <div class="w-16 h-16 bg-amber-50 text-amber-600 rounded-2xl flex-shrink-0 flex items-center justify-center text-3xl shadow-inner relative z-10">💡</div>
                <div class="relative z-10 text-center md:text-left">
                    <h4 class="font-black text-gray-900 text-xs uppercase tracking-widest mb-2">Petunjuk Pengisian</h4>
                    <p class="text-[11px] text-gray-500 font-medium leading-relaxed max-w-xl">
                        Selalu gunakan font <b>Arial</b> atau <b>Times New Roman</b> sesuai pedoman. Pastikan nomor surat sudah di-generate melalui SIAP Dakwah sebelum mencetak dokumen final.
                    </p>
                </div>
            </div>
        </div>
        ...
        {{-- GRID --}}
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" id="templateGrid">
            @forelse($templates as $tmpl)
            <div class="template-card bg-white p-8 rounded-[40px] border border-gray-100 hover:border-red-200 hover:shadow-2xl hover:shadow-red-100/20 transition-all duration-500 group flex flex-col justify-between transform hover:-translate-y-2 animasi-kotak delay-200">
                <div>
                    <div class="flex justify-between items-start mb-8">
                        ...
                        <span class="bg-gray-50 text-gray-400 text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-gray-100">Standardized</span>
                    </div>
                    <h4 class="template-title font-black text-gray-900 text-xl mb-3 uppercase tracking-tight group-hover:text-red-700 transition leading-tight">{{ $tmpl->judul }}</h4>
                    <p class="text-[11px] text-gray-400 mb-10 font-medium leading-relaxed line-clamp-3 group-hover:text-gray-600 transition">{{ $tmpl->deskripsi }}</p>
                </div>
                
                <a href="{{ $tmpl->link }}" target="_blank" class="w-full text-center py-4 rounded-2xl bg-gray-900 text-white font-black text-[10px] uppercase tracking-[0.3em] hover:bg-red-700 transition shadow-xl shadow-gray-200 active:scale-95 flex items-center justify-center gap-2">
                    Unduh Template
            ...
            <div class="col-span-3 py-32 text-center opacity-30">
                <p class="text-gray-400 font-black uppercase tracking-widest text-sm">Database Sedang Diperbarui</p>
            </div>
            @endforelse
        </div>
    </main>
...
document.getElementById('templateSearch').addEventListener('input', function(e) {
    ...
    cards.forEach(card => {
        const title = card.querySelector('.template-title').innerText.toLowerCase();
